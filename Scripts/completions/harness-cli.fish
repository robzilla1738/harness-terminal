...
set -l __harness_cli_subcommands doctor version color-check theme-preview ping daemon-stats list-workspaces list-surfaces list-sessions list-windows list-panes list-agents list-clients has-session list-commands get-snapshot new-workspace new-session new-tab new-split select-workspace select-session select-tab select-pane close-tab close-session promote-session demote-session kill-pane swap-pane resize-pane zoom-pane break-pane join-pane move-pane respawn-pane rotate-window select-layout next-layout previous-layout renumber-windows rename-tab rename-session rename-workspace link-window unlink-window send send-keys capture-pane pipe-pane copy-mode attach attach-window record replay control-mode set-buffer list-buffers show-buffer delete-buffer paste-buffer save-buffer load-buffer set-option show-options set-environment show-environment bind-hook unbind-hook list-hooks wait-for bind-key unbind-key list-keys detect-agent notify detach-client display-message install install-hooks install-shell-integration completions remote daemon
...
complete -c harness-cli -n "__fish_seen_subcommand_from doctor version daemon-stats list-workspaces list-surfaces list-sessions list-windows list-panes list-agents list-clients list-buffers show-options show-environment list-hooks" -l json   -d "Machine-readable JSON output"
complete -c harness-cli -n "__fish_seen_subcommand_from doctor version daemon-stats list-workspaces list-surfaces list-sessions list-windows list-panes list-agents list-clients list-buffers show-options show-environment list-hooks" -l pretty -d "Indent JSON output (with --json)"
