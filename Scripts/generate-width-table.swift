...
//
// Run manually (NOT part of the build) whenever the canonical ranges change:
//
...
//
// ## Where the canonical ranges come from
//
// The ranges below are DERIVED FROM THE UNICODE CHARACTER DATABASE (currently 15.1), not
// hand-curated. Derivation:
//
//   wide  = East_Asian_Width ∈ {W, F}            (UAX #11, EastAsianWidth.txt)
//           ∪ default-Wide unassigned gaps in the CJK ideograph blocks/planes
//             (3400–4DBF, 4E00–9FFF, F900–FAFF, 20000–2FFFD, 30000–3FFFD)
//   zero  = General_Category ∈ {Mn, Me, Cf, Zl, Zp} for cp ≥ 0x0300
//             (DerivedGeneralCategory.txt)
//
// Deliberate deviations (mirrored in CharacterWidth.swift's type doc):
//   - cp < 0x300 is owned by the tier-1 fast path (controls 0, rest 1) — soft hyphen stays 1.
//   - Hangul conjoining jamo V/T (1160–11FF, D7B0–D7FF) stay width 1 until the grapheme
//     layer composes syllables (attachCombining folds only Grapheme_Extend scalars).
//   - Regional indicators stay width 1 per scalar (flag pairing is grapheme-level).
//
// To move to a NEW Unicode version: re-derive the two range lists from the UCD files above
// (any UCD-complete environment works — `python3 -c "import unicodedata"` ships 15.1 era data;
// or parse the .txt files directly), replace the range literals BOTH here and in
// `CharacterWidth.swift`, then re-run this script. `CharacterWidthTests` fails CI until the
// table, the reference ranges, and this generator agree byte-for-byte over every scalar.
...
let zeroWidthRanges: [ClosedRange<UInt32>] = [
        ...
        0x2028 ... 0x202E, 0x2060 ... 0x2064, 0x2066 ... 0x206F, 0x20D0 ... 0x20F0,
