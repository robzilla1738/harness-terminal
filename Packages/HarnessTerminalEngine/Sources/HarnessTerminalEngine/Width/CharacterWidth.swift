...
///
/// The canonical ranges below are **derived from the Unicode Character Database** (15.1):
...
///
/// Regenerate with `Scripts/generate-width-table.swift` (see its header for the workflow);
...
public enum CharacterWidth {
    ...
    // scalar by `CharacterWidthTests`. The ranges are themselves UCD-generated (see the type doc);
    // to change them, edit the derivation in `Scripts/generate-width-table.swift`, re-run it, and
    // paste its emitted canonical ranges here. The parity test fails until both are regenerated.
    ...
    /// UCD-derived zero-width scalars (General_Category Mn/Me/Cf/Zl/Zp, cp ≥ 0x0300).
    /// Sorted, non-overlapping, maximal runs. GENERATED — do not hand-edit (see type doc).
    ...
    /// UCD-derived wide scalars (East_Asian_Width W/F + default-Wide CJK gaps).
    /// Sorted, non-overlapping, maximal runs. GENERATED — do not hand-edit (see type doc).
